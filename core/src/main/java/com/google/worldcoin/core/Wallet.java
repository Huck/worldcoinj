...
import static com.google.worldcoin.core.Utils.worldcoinValueToPlainString;
...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
    ...
    private WorldcoinWalletVersion version;
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s WDC from our own wallet," +
                        " and sends us %s WDC", tx.getHashAsString(), Utils.worldcoinValueToFriendlyString(valueSentFromMe),
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
        ...
        log.info("Received tx{} for {} WDC: {} [{}] in block {}", sideChain ? " on a side chain" : "",
                worldcoinValueToFriendlyString(valueDifference), tx.getHashAsString(), relativityOffset,
        ...
        if (!insideReorg && bestChain) {
            ...
            log.info("Balance is now: " + worldcoinValueToFriendlyString(newBalance));
            if (!wasPending) {
    ...
    public boolean completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            // We need to know if we need to add an additional fee because one of our values are smaller than 0.01 WDC
            ...
            if (bestChangeOutput != null) {
                ...
                log.info("  with {} coins change", worldcoinValueToFriendlyString(bestChangeOutput.getValue()));
            }
            final BigInteger calculatedFee = totalInput.subtract(totalOutput);
            if (calculatedFee.compareTo(BigInteger.ZERO) > 0) {
                log.info("  with a fee of {}", worldcoinValueToFriendlyString(calculatedFee));
            }
    ...
    public boolean completeTx(SendRequest req, boolean sign) {
        ...
        try {
            ...
            // Keep a track of the date the tx was created (used in WorldcoinWalletService
    ...
    private boolean adjustOutputDownwardsForFee(Transaction tx, CoinSelection coinSelection, BigInteger baseFee, BigInteger feePerKb) {
        ...
        int size = tx.worldcoinSerialize().length;
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s WDC in:%n",
                    worldcoinValueToPlainString(balance)));
    ...
    public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks, List<StoredBlock> newBlocks) throws VerificationException {
        ...
        try {
            ...
            log.info("post-reorg balance is {}", Utils.worldcoinValueToFriendlyString(balance));
     ...
     */
    public WorldcoinWalletVersion getVersion() {
        return version;
    }
     ...
     */
    public void setVersion(WorldcoinWalletVersion version) {
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            while (true) {
                ...
                if (change.compareTo(BigInteger.ZERO) > 0) {
                    ...
                    if (req.ensureMinRequiredFee && Transaction.MIN_NONDUST_OUTPUT.compareTo(change) >= 0) {
                        ...
                        size += changeOutput.worldcoinSerialize().length + VarInt.sizeOf(req.tx.getOutputs().size()) - VarInt.sizeOf(req.tx.getOutputs().size() - 1);
                ...
                size += req.tx.worldcoinSerialize().length;
            ...
            if (selection3 == null && selection2 == null && selection1 == null) {
                ...
                log.warn("Insufficient value in wallet for send: needed {} more", worldcoinValueToFriendlyString(valueMissing));
    ...
    private Transaction rekeyOneBatch(long keyRotationTimestamp, final TransactionBroadcaster broadcaster) {
        ...
        try {
            ...
            checkState(rekeyTx.worldcoinSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            commitTx(rekeyTx);
