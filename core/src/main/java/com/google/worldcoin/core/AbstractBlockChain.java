...
public abstract class AbstractBlockChain {
    ...
    private boolean IsSuperMajority(int minVersion, Sha256Hash pstart, int nRequired, int nToCheck)
    {
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        boolean fNewDifficultyProtocol = (nHeight >= NetworkParameters.nDiffChangeTarget || params.getId().equals(NetworkParameters.ID_TESTNET));

        long retargetTimespan = NetworkParameters.TARGET_TIMESPAN_V1;
        long retargetSpacing = NetworkParameters.TARGET_SPACING_V1;
        long retargetInterval =  params.getInterval_v1();

        if(fNewDifficultyProtocol) {
            retargetTimespan = NetworkParameters.TARGET_TIMESPAN_V2;
            retargetSpacing = NetworkParameters.TARGET_SPACING_V2;
        ...
        long goBack = retargetInterval - 1;
        ...
        if(nextBlock.getVersion() < 2){
            if ((!params.getId().equals(NetworkParameters.ID_TESTNET) && IsSuperMajority(2, prev.getHash(), 950, 1000)) ||
            (params.getId().equals(NetworkParameters.ID_TESTNET) && IsSuperMajority(2, prev.getHash(), 75, 100)))
            {
                throw new VerificationException("AcceptBlock() : rejected nVersion=1 block");
            }

        }
        if(nextBlock.getVersion() >= 2){
            if ((!params.getId().equals(NetworkParameters.ID_TESTNET) && IsSuperMajority(2, prev.getHash(), 750, 1000)) ||
            (params.getId().equals(NetworkParameters.ID_TESTNET) && IsSuperMajority(2, prev.getHash(), 51, 100)))
            {
                //TODO: worldcoin, this below was in the c++ clinet, do we need it, how?
                /*CScript expect = CScript() << nHeight;
                if (vtx[0].vin[0].scriptSig.size() < expect.size() ||
                    !std::equal(expect.begin(), expect.end(), vtx[0].vin[0].scriptSig.begin()))
                throw new VerificationException("AcceptBlock() : block height mismatch in coinbase");*/
            }
        ...
        if(fNewDifficultyProtocol) {
            if (timespan < (retargetTimespan - (retargetTimespan/10)) ) timespan = (retargetTimespan - (retargetTimespan/10));
            if (timespan > (retargetTimespan + (retargetTimespan/10)) ) timespan = (retargetTimespan + (retargetTimespan/10));
        }
        else {
            if (timespan < retargetTimespan/4) timespan = retargetTimespan/4;
            if (timespan > retargetTimespan*4) timespan = retargetTimespan*4;
        }

        BigInteger newDifficulty = Utils.decodeCompactBits(prev.getDifficultyTarget()); // == to SetCompact
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // goes backwards.
        if (timeDelta >= 0 && timeDelta <= NetworkParameters.TARGET_SPACING_V2 * 2) {
