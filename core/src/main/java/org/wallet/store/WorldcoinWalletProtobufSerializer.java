 ...
 * format is defined by the <tt>worldcoin.proto</tt> file in the worldcoinj source distribution.<p>
 *
 ...
 */
public class WorldcoinWalletProtobufSerializer extends WalletProtobufSerializer {
    private static final Logger log = LoggerFactory.getLogger(WorldcoinWalletProtobufSerializer.class);

    // Early version of name-value value for use in protecting encrypted wallets from being loaded
    // into earlier versions of WorldcoinWallet. Unfortunately I merged this into the WorldcoinWallet v0.4 code by mistake.
    // @deprecated replaced by ORG_WORLDCOIN_WALLET_WALLET_PROTECT_2
    public static final String ORG_WORLDCOIN_WALLET_WALLET_PROTECT = "org.wallet.walletProtect";

    public static final String ORG_WORLDCOIN_WALLET_WALLET_PROTECT_2 = "org.wallet.walletProtect.2";

    public WorldcoinWalletProtobufSerializer() {
        super();
    }
        ...
        if (walletProto.hasVersion()) {
            ...
            if (version == 0) {
                if (walletEncryptionType == EncryptionType.ENCRYPTED_SCRYPT_AES) {
                    ...
                    wallet.setVersion(WorldcoinWalletVersion.PROTOBUF_ENCRYPTED);
                } else if (walletEncryptionType == EncryptionType.UNENCRYPTED) {
                    // If it is unencrypted it is protobuf.2
                    wallet.setVersion(WorldcoinWalletVersion.PROTOBUF);
                } else {
            ...
            } else if (version == WorldcoinWalletVersion.PROTOBUF.getWalletVersionAsInt()) {
                wallet.setVersion(WorldcoinWalletVersion.PROTOBUF);
            } else {
                if (version == WorldcoinWalletVersion.PROTOBUF_ENCRYPTED.getWalletVersionAsInt()) {
                    wallet.setVersion(WorldcoinWalletVersion.PROTOBUF_ENCRYPTED);
                } else {
        ...
        for (Protos.Extension extProto : walletProto.getExtensionList()) {
            ...
            if (extension == null) {
                if (extProto.getMandatory()) {
                    // If the extension is the ORG_WORLDCOIN_WALLET_WALLET_PROTECT or ORG_WORLDCOIN_WALLET_WALLET_PROTECT_2 then we know about that.
                    // This is a marker extension to prevent earlier versions of wallet loading encrypted wallets.
                    
                    // Unfortunately I merged the recognition of the ORG_WORLDCOIN_WALLET_WALLET_PROTECT mandatory extension into the v0.4 code
                    // so it could load encrypted wallets mistakenly.
                    
                    // Hence the v0.5 code now writes ORG_WORLDCOIN_WALLET_WALLET_PROTECT_2.
                    if (!(extProto.getId().equals(WorldcoinWalletProtobufSerializer.ORG_WORLDCOIN_WALLET_WALLET_PROTECT) ||
                            extProto.getId().equals(WorldcoinWalletProtobufSerializer.ORG_WORLDCOIN_WALLET_WALLET_PROTECT_2))) {
    ...
    protected void readTransaction(Protos.Transaction txProto, NetworkParameters params) throws UnreadableWalletException {
        ...
        // (This code is is here because some old WorldcoinWallet serialised wallets had the same tx appearing twice and the wallets would not load).
    ...
    protected WalletTransaction connectTransactionOutputs(org.worldcoinj.wallet.Protos.Transaction txProto) throws UnreadableWalletException {
