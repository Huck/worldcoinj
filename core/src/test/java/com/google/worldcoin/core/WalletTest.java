...
public class WalletTest extends TestWithWallet {
    ...
    public void blockChainCatchup() throws Exception {
        ...
        Threading.waitForUserCode();
        assertEquals(worldcoinValueToFriendlyString(wallet.getBalance()), "0.90");
        assertEquals(null, txn[0]);
        assertEquals(2, confTxns.size());
        assertEquals(txn[1].getHash(), send1.getHash());
        assertEquals(worldcoinValueToFriendlyString(bigints[2]), "1.00");
        assertEquals(worldcoinValueToFriendlyString(bigints[3]), "0.90");
        ...
        assertEquals(worldcoinValueToFriendlyString(wallet.getBalance()), "0.80");
        Threading.waitForUserCode();
    ...
    public void isConsistent_duplicates() throws Exception {
        ...
        Transaction txClone = new Transaction(params, tx.worldcoinSerialize());
        try {
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        send2 = new Transaction(params, send2.worldcoinSerialize());
    ...
    public void watchingScriptsBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(1e-12).contains(outPoint.worldcoinSerialize()));

        wallet.receiveFromBlock(t1, b1, BlockChain.NewBlockType.BEST_CHAIN, 0);
        assertTrue(wallet.getBloomFilter(1e-12).contains(outPoint.worldcoinSerialize()));
    }

    @Test
    public void autosaveImmediate() throws Exception {
        // Test that the wallet will save itself automatically when it changes.
        File f = File.createTempFile("worldcoinj-unit-test", null);
    ...
    public void feeSolverAndCoinSelectionTest() throws Exception {
        ...
        assertTrue(request15.tx.worldcoinSerialize().length > 1000);
        ...
        assertTrue(request16.tx.worldcoinSerialize().length > 1000);
        ...
        int theoreticalMaxLength17 = request17.tx.worldcoinSerialize().length + myKey.getPubKey().length + 75;
        ...
        Transaction spend17 = request17.tx;
        {
            ...
            final int length = spend17.worldcoinSerialize().length;
        ...
        int theoreticalMaxLength18 = spend18.worldcoinSerialize().length + myKey.getPubKey().length + 75;
        ...
        assertTrue(spend18.worldcoinSerialize().length >= 998);
        assertTrue(spend18.worldcoinSerialize().length <= 1001);
        ...
        assertTrue(request26.tx.worldcoinSerialize().length > 1000);
    ...
    public void basicCategoryStepTest() throws Exception {
        ...
        request4.feePerKb = Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.divide(BigInteger.valueOf(request3.tx.worldcoinSerialize().length));
        wallet.completeTx(request4);
    ...
    public void testEmptyWallet() throws Exception {
        ...
        // Add exactly 0.01 + transaction 0.01
        StoredBlock block = new StoredBlock(makeSolvedTestBlock(blockStore, outputKey), BigInteger.ONE, 1);
        Transaction tx = createFakeTx(params, Utils.toNanoCoins(0, 2), myAddress);
        wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        ...
        tx = createFakeTx(params, Utils.toNanoCoins(0, 2), myAddress);
        wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        tx = createFakeTx(params, Utils.toNanoCoins(0, 2), myAddress);
        ...
        tx = createFakeTx(params, Utils.toNanoCoins(0, 2).subtract(BigInteger.ONE), myAddress);
        wallet.receiveFromBlock(tx, block2, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        request = SendRequest.emptyWallet(outputKey);
        wallet.completeTx(request);
        wallet.commitTx(request.tx);
        assertEquals(BigInteger.ZERO, wallet.getBalance());
        assertEquals(Utils.toNanoCoins(0, 2).subtract(BigInteger.ONE).subtract(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE), request.tx.getOutput(0).getValue());
